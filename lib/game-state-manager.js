...
class GameStateManager extends EventEmitter {
  ...
  async placeBet(poolId, userId, amount, guess, updatePointsFn) {
    ...
    try {
      ...
      const resolvedUpdatePointsFn = this.getUpdatePointsFn(updatePointsFn, game);

      // Fixed bet size (all-in if player has less)
      const betAmount = Math.min(this.BET_AMOUNT, player.points);
      if (betAmount <= 0) {
        console.error(`[GameState] Insufficient points: ${player.points}`);
        throw new Error('Insufficient points');
      }

      if (amount !== betAmount) {
        console.log(`[GameState] Adjusting bet amount from ${amount} to ${betAmount}`);
      }
      ...
      // Update in database
      await resolvedUpdatePointsFn(userId, player.points);

      // Store bet
      game.bets[userId] = { amount: betAmount, guess, placed: Date.now() };

      // Cache update function for timeouts
      game.updatePointsFn = resolvedUpdatePointsFn;
      ...
      if (betsPlaced >= bettingPlayers.length) {
        ...
        await this.revealRound(poolId, resolvedUpdatePointsFn);
      }
  ...
  async revealRound(poolId, updatePointsFn) {
    ...
    const bets = Object.entries(game.bets).map(([userId, bet]) => ({
      userId,
      bet
    }));
    const totalPot = bets.reduce((sum, b) => sum + (b.bet.amount || 0), 0);
    const winningBets = bets.filter(b => b.bet.guess === correctChoice);
    const totalWinningBet = winningBets.reduce((sum, b) => sum + (b.bet.amount || 0), 0);

    if (totalPot > 0 && totalWinningBet > 0) {
      const allocations = winningBets.map(b => {
        const rawShare = (b.bet.amount / totalWinningBet) * totalPot;
        const floorShare = Math.floor(rawShare);
        return {
          userId: b.userId,
          betAmount: b.bet.amount,
          floorShare,
          fractional: rawShare - floorShare
        };
      });

      let remainder = totalPot - allocations.reduce((sum, a) => sum + a.floorShare, 0);
      allocations.sort((a, b) => b.fractional - a.fractional || b.betAmount - a.betAmount);
      for (let i = 0; i < allocations.length && remainder > 0; i += 1) {
        allocations[i].floorShare += 1;
        remainder -= 1;
      }

      for (const alloc of allocations) {
        const player = game.players.find(p => p.userId === alloc.userId);
        if (!player) continue;
        const winnings = alloc.floorShare;
        player.points += winnings;
        winners.push({ userId: alloc.userId, name: player.name, won: winnings });
        await resolvedUpdatePointsFn(alloc.userId, player.points);
      }
    }
  ...
  async handleBettingTimeout(poolId) {
    ...
    const resolvedUpdatePointsFn = this.getUpdatePointsFn(null, game);
    ...
    for (const player of bettingPlayers) {
      if (!game.bets[player.userId]) {
        // Auto-bet fixed amount (all-in if less) on 'B'
        const autoBetAmount = Math.min(this.BET_AMOUNT, player.points);
        if (autoBetAmount > 0) {
          player.points -= autoBetAmount;
          await resolvedUpdatePointsFn(player.userId, player.points);
  ...
  setGlobalUpdatePointsFn(fn) {
    if (typeof fn === 'function') {
      this.globalUpdatePointsFn = fn;
    }
  }

  getUpdatePointsFn(updatePointsFn, game) {
    return updatePointsFn || game?.updatePointsFn || this.globalUpdatePointsFn || (async () => {});
  }
  ...
  createPool(poolId, ownerId) {
    ...
    console.log(`[GameState] Created pool ${poolId} (owner ${ownerId})`);
    return game;
  }
   ...
   */
  addPlayer(poolId, userId, socketId, name, points) {
    const game = this.games.get(poolId);
    if (!game) throw new Error('Pool not found');

    if (game.players.some(p => p.userId === userId)) {
      // update socket mapping if re-joining
    ...
    const player = {
      ...
      points: typeof points === 'number' ? points : this.INITIAL_POINTS,
      isConnected: !!socketId,
      rank: null
    };

    game.players.push(player);
    this.playerSockets.set(userId, socketId || null);

    console.log(`[GameState] Player ${player.name} (${userId}) added to pool ${poolId}`);
    return game;
  }

  /**
   * Start a game (admin or auto-start)
   */
  startGame(poolId) {
    const game = this.games.get(poolId);
    if (!game) throw new Error('Pool not found');
    ...
    game.bets = {};
    game.paused = false;
    game.nextQuestionIndex = 0;

    // Kick off first round
    this.startNewRound(poolId);

    console.log(`[GameState] Game started in pool ${poolId}`);
    return game;
  }

  /**
   * Pause current game timer and return remaining ms
   */
  pauseGame(poolId) {
    const game = this.games.get(poolId);
    if (!game) throw new Error('Pool not found');

    if (!game.timer && !game.timerEndsAt) {
      game.timerPaused = true;
      return 0;
    }
    ...
    console.log(`[GameState] Paused pool ${poolId}. Remaining: ${remaining}ms`);
    return remaining;
  }
